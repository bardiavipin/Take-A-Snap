		...
		var LogoEditor = A.Component.create(
			{
				...
				prototype: {
					...
					renderUI: function() {
						var instance = this;
						console.log("LOGO EDITOR OVERRIDDEN...");
						...
						instance._capture = instance.one('#captureme');
					},
					...
					bindUI: function() {
						...
						if(instance._capture){
							instance._capture.on('change', instance._onCapture, instance);	
						}						
					},
					...
					_onCapture: function(event) {
						...
						if (event._currentTarget.files && event._currentTarget.files[0]) {
				            var reader = new FileReader();
				            
				            reader.onload = function (e) {

				            	var portraitPreviewImg = instance._portraitPreviewImg;
				            	portraitPreviewImg.attr('height', "200");
				            	portraitPreviewImg.attr('width', "200");
				            	portraitPreviewImg.attr('src', e.target.result);
				            }
				            
				            reader.readAsDataURL(event._currentTarget.files[0]);
